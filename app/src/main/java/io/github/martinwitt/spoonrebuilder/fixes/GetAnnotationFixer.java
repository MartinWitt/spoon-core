...
public class GetAnnotationFixer extends AbstractProcessor<CtMethod<?>> {
  ...
  public void process(CtMethod<?> element) {
    if (element.getSimpleName().equals("getAnnotation")
        && element.getDeclaringType().getSimpleName().equals("CtElementImpl") && hasClassParameter(element)) {
      element.getElements(new TypeFilter<>(CtReturn.class)).stream()
          .map(v -> v.getReturnedExpression())
          .filter(v -> !v.toString().contains("null")).forEach(v -> {
  ...
  private boolean hasClassParameter(CtMethod<?> element) {
    return element.getParameters().stream().allMatch(v -> v.getType().getSimpleName().equals("Class"));
  }
