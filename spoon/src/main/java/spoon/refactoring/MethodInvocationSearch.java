...
public class MethodInvocationSearch extends CtScanner {
	...
	@Override
	public <T> void visitCtMethod(CtMethod method) {
		...
		if (method instanceof CtLambda) {
			...
			transformedMethod = method.getParent(spoon.reflect.declaration.CtExecutable.class);
		} else {
			transformedMethod = method;
		}
		List<CtInvocation> invocations = method.getElements(new TypeFilter<>(spoon.reflect.code.CtInvocation.class));
		List<CtConstructorCall> constructors = method.getElements(new TypeFilter<>(spoon.reflect.code.CtConstructorCall.class));
		...
		invocations.stream().filter(v -> !v.isImplicit()).map(v -> v.getExecutable().getExecutableDeclaration()).filter(Objects::nonNull).filter(v -> v.getPosition().isValidPosition()).forEach(v -> invocationsOfMethod.merge(v, new HashSet<>(Arrays.asList(transformedMethod)), (o1, o2) -> Stream.concat(o1.stream(), o2.stream()).collect(Collectors.toCollection(HashSet::new))));
		constructors.stream().filter(v -> !v.isImplicit()).map(v -> v.getExecutable().getExecutableDeclaration()).filter(Objects::nonNull).forEach(v -> invocationsOfMethod.merge(v, new HashSet<>(Arrays.asList(transformedMethod)), (o1, o2) -> Stream.concat(o1.stream(), o2.stream()).collect(Collectors.toCollection(HashSet::new))));
	...
	@Override
	public <T> void visitCtField(CtField field) {
		field.getElements(new TypeFilter<>(spoon.reflect.code.CtInvocation.class)).stream().map(call -> call.getExecutable().getExecutableDeclaration()).forEach(method -> invocationsOfField.merge(method, new HashSet<>(Arrays.asList(field.getDeclaringType())), (o1, o2) -> Stream.concat(o1.stream(), o2.stream()).collect(Collectors.toCollection(HashSet::new))));
		field.getElements(new TypeFilter<>(spoon.reflect.code.CtConstructorCall.class)).stream().map(call -> call.getExecutable().getExecutableDeclaration()).forEach(method -> invocationsOfField.merge(method, new HashSet<>(Arrays.asList(field.getDeclaringType())), (o1, o2) -> Stream.concat(o1.stream(), o2.stream()).collect(Collectors.toCollection(HashSet::new))));
