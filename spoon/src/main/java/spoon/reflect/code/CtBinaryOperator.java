 ...
 */
public interface CtBinaryOperator extends CtExpression {
	...
	<C extends CtBinaryOperator> C setLeftHandOperand(CtExpression expression);

	/**
	 * Sets the right-hand operand.
	 */
	@PropertySetter(role = RIGHT_OPERAND)
	<C extends CtBinaryOperator> C setRightHandOperand(CtExpression expression);

	/**
	 * Sets the kind of this binary operator.
	 */
	@PropertySetter(role = OPERATOR_KIND)
	<C extends CtBinaryOperator> C setKind(BinaryOperatorKind kind);
