...
public class CodeFactory extends SubFactory {
	...
	public <T> CtCatchVariable createCatchVariable(CtTypeReference type, String name, ModifierKind...modifierKinds) {
		return factory.Core().<T>createCatchVariable().<CtCatchVariable>setSimpleName(name).<CtCatchVariable>setType(type).setModifiers(new HashSet<>(Arrays.asList(modifierKinds)));
	}
