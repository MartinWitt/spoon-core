 ...
 */
public interface CtFieldReference extends CtVariableReference {
	/**
	...
	<C extends CtFieldReference> C setDeclaringType(CtTypeReference declaringType);

	/**
	 * Forces a reference to a final element.
	 */
	@PropertySetter(role = CtRole.IS_FINAL)
	<C extends CtFieldReference> C setFinal(boolean b);

	/**
	 * Forces a reference to a static element.
	 */
	@PropertySetter(role = CtRole.IS_STATIC)
	<C extends CtFieldReference> C setStatic(boolean b);
