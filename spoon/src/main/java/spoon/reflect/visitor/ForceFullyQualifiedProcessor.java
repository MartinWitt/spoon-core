...
public class ForceFullyQualifiedProcessor extends ImportAnalyzer<LexicalScope> {@Override
	...
	protected void handleTypeReference(CtTypeReference reference, LexicalScope nameScope, CtRole role) {
		if (reference.isSimplyQualified() || reference.isImplicit()) {
			...
			if (isTypeReferenceToEnclosingType(nameScope, reference) && reference.getParent(spoon.reflect.declaration.CtAnonymousExecutable.class) != null) {
	...
	private boolean isThisAccess(CtTypeReference typeRef) {
		return getParentIfType(getParentIfType(typeRef, spoon.reflect.code.CtTypeAccess.class), spoon.reflect.code.CtThisAccess.class) != null;
	}

	private boolean isThisAccess(CtTypeAccess typeAccess) {
		return getParentIfType(typeAccess, spoon.reflect.code.CtThisAccess.class) != null;
	}
}
