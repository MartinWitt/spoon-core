...
public class ImportConflictDetector extends ImportAnalyzer<LexicalScope> {@Override
	...
	protected void handleTypeReference(CtTypeReference ref, LexicalScope nameScope, CtRole role) {
		if (ref.isImplicit()) {
			...
			CtTargetedExpression targetedExpr = getParentIfType(getParentIfType(ref, spoon.reflect.code.CtTypeAccess.class), spoon.reflect.code.CtTargetedExpression.class);
			if (targetedExpr instanceof CtInvocation) {
				...
				if (targetedExpr instanceof CtFieldAccess) {
