...
public class JDTTreeBuilder extends ASTVisitor {
	...
	abstract// an abstract class here is better because the method is actually package-protected, as the type, (and not public as in the case of interface methods in Java)
	...
	static CompilationUnit getOrCreateCompilationUnit(CompilationUnitDeclaration compilationUnitDeclaration, Factory factory) {
		...
		public boolean visit(AnnotationMethodDeclaration annotationMethodDeclaration, ClassScope classScope) {
			CtAnnotationMethod ctAnnotationMethod = factory.Core().createAnnotationMethod();
			...
			if (annotationMethodDeclaration.binding != null) {
				ctAnnotationMethod.setExtendedModifiers(getModifiers(annotationMethodDeclaration.binding.modifiers, true, ModifierTarget.METHOD));
			}
			Set<CtExtendedModifier> explicitModifiers = getModifiers(annotationMethodDeclaration.modifiers, false, ModifierTarget.METHOD);
		...
		public boolean visit(Argument argument, BlockScope scope) {
			...
			CtParameter p = helper.createParameter(argument);
			if (isVar) {
				p.setInferred(true);
			}
		...
		public boolean visit(ArrayTypeReference arrayTypeReference, BlockScope scope) {
			CtTypeReference objectCtTypeReference = references.buildTypeReference(arrayTypeReference, scope);
			final CtTypeAccess typeAccess = factory.Code().createTypeAccess(objectCtTypeReference);
		...
		public boolean visit(ArrayQualifiedTypeReference arrayQualifiedTypeReference, BlockScope scope) {
			...
			CtArrayTypeReference arrayType = ((CtArrayTypeReference) (references.getTypeReference(arrayQualifiedTypeReference.resolvedType)));
	...
	private boolean visitTypeParameter(TypeParameter typeParameter, Scope scope) {
		...
		public boolean visit(ExplicitConstructorCall explicitConstructor, BlockScope scope) {
			...
			inv.getExecutable().setType(declaringType == null ? null : ((CtTypeReference) (declaringType.clone())));
	...
	private boolean createParameterizedType(TypeReference parameterizedTypeReference) {
		...
		public boolean visit(QualifiedTypeReference qualifiedTypeReference, BlockScope scope) {
			...
			if (context.stack.peekFirst().node instanceof UnionTypeReference) {
				CtTypeReference reference = references.<Throwable>getTypeReference(qualifiedTypeReference.resolvedType);
				if (reference == null) {
				...
				if (context.stack.peekFirst().element instanceof CtCatch) {
		...
		public boolean visit(SingleTypeReference singleTypeReference, BlockScope scope) {
			...
			if (context.stack.peekFirst().node instanceof UnionTypeReference) {
				...
				if (context.stack.peekFirst().element instanceof CtCatch) {
					...
					if (context.stack.getFirst().element instanceof CtExecutableReferenceExpression) {
