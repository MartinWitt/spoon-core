...
public class JDTTreeBuilderHelper {
	...
	CtCatchVariable createCatchVariable(TypeReference typeReference, Scope scope) {
		...
		CtCatchVariable result = jdtTreeBuilder.getFactory().Core().createCatchVariable();
	 ...
	 */
	public <T> CtVariableAccess createVariableAccess(CtVariableReference variableReference, boolean isReadAccess) {
	 ...
	 */
	<T> CtVariableAccess createVariableAccess(SingleNameReference singleNameReference) {
		CtVariableAccess va;
	 ...
	 */
	<T> CtVariableAccess createVariableAccessNoClasspath(SingleNameReference singleNameReference) {
		...
		final CtVariable variable = contextBuilder.getVariableDeclaration(name);
		if (variable == null) {
			return null;
		}

		final CtVariableReference variableReference;
		final CtVariableAccess variableAccess;
		if (variable instanceof CtParameter) {
			// create variable of concrete type to avoid type casting while calling methods
			final CtParameterReference parameterReference = coreFactory.createParameterReference();
	 ...
	 * @return a variable access.
	 */
	<T> CtVariableAccess createVariableAccess(QualifiedNameReference qualifiedNameReference) {
		...
		if (qualifiedNameReference.binding instanceof FieldBinding) {
			...
			va = createFieldAccess(ref, createTargetFieldAccess(qualifiedNameReference, ((CtFieldReference) ( ref))), isOtherBinding && fromAssignment);
		} else {
		...
		if (qualifiedNameReference.otherBindings != null) {
			...
			for (FieldBinding b : qualifiedNameReference.otherBindings) {
				...
				CtFieldAccess other = createFieldAccess(
	 ...
	 * in the {@link spoon.reflect.factory.CodeFactory} if you think that is a good idea.
	 */
	public <T> CtFieldAccess createFieldAccess(CtVariableReference variableReference, CtExpression target, boolean isReadAccess) {
	 ...
	 */
	<T> CtFieldAccess createFieldAccess(SingleNameReference singleNameReference) {
		CtFieldAccess va;
		if (isLhsAssignment(jdtTreeBuilder.getContextBuilder(), singleNameReference)) {
		...
		if (va.getVariable() != null) {
			final CtFieldReference ref = va.getVariable();
	 ...
	 * @return a field access.
	 */
	<T> CtFieldAccess createFieldAccessNoClasspath(SingleNameReference singleNameReference) {
		CtFieldAccess va;
		if (isLhsAssignment(jdtTreeBuilder.getContextBuilder(), singleNameReference)) {
			va = jdtTreeBuilder.getFactory().Core().createFieldWrite();
		} else {
		...
		if (declaring instanceof CtTypeReference && va.getVariable() != null) {
			final CtTypeReference declaringRef = ((CtTypeReference) ( declaring));
	 ...
	 * @return a field access.
	 */
	<T> CtFieldAccess createFieldAccessNoClasspath(QualifiedNameReference qualifiedNameReference) {
		boolean fromAssignment = isLhsAssignment(jdtTreeBuilder.getContextBuilder(), qualifiedNameReference);
		CtFieldAccess fieldAccess = createFieldAccess(jdtTreeBuilder.getReferencesBuilder().<T>getVariableReference((ProblemBinding) qualifiedNameReference.binding), null, fromAssignment);
		...
		final CtTypeReference declaringRef = jdtTreeBuilder.getReferencesBuilder().getTypeReference(declaringType);
	 ...
	 * @return a field access.
	 */
	<T> CtFieldAccess createFieldAccess(FieldReference fieldReference) {
		CtFieldAccess fieldAccess;
	...
	CtTypeAccess createTypeAccess(QualifiedNameReference qualifiedNameReference, CtFieldReference fieldReference) {
		...
		if (receiverType != null) {
			final CtTypeReference qualifiedRef = jdtTreeBuilder.getReferencesBuilder().getQualifiedTypeReference(
	 ...
	 * @return a type access.
	 */
	<T> CtTypeAccess createTypeAccessNoClasspath(QualifiedNameReference qualifiedNameReference) {
		...
		final CtTypeAccess typeAccess = jdtTreeBuilder.getFactory().Code().createTypeAccess(typeReference);
	 ...
	 * @return a type access.
	 */
	<T> CtTypeAccess createTypeAccessNoClasspath(SingleNameReference singleNameReference) {
		final CtTypeReference typeReference = jdtTreeBuilder.getFactory().Core().createTypeReference();
	 ...
	 */
	CtExpression createTargetFieldAccess(QualifiedNameReference qualifiedNameReference, CtFieldReference ref) {
	 ...
	 * @return a parameter.
	 */
	<T> CtParameter createParameter(Argument argument) {
		CtParameter p = jdtTreeBuilder.getFactory().Core().createParameter();
	 ...
	 */
	<T, E extends CtExpression> CtExecutableReferenceExpression createExecutableReferenceExpression(ReferenceExpression referenceExpression) {
		CtExecutableReferenceExpression executableRef = jdtTreeBuilder.getFactory().Core().createExecutableReferenceExpression();
		CtExecutableReference executableReference = jdtTreeBuilder.getReferencesBuilder().getExecutableReference(referenceExpression.binding);
		...
		final CtTypeReference returnType = executableReference.getType();
