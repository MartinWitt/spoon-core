...
public class CtLocalVariableImpl extends CtStatementImpl implements CtLocalVariable {
	...
	@Override
	public <C extends CtVariable> C setDefaultExpression(CtExpression defaultExpression) {
	...
	public boolean isPartOfJointDeclaration() {
		...
		for (Object o : getParent().getDirectChildren()) {
			...
			if (f.getPosition().getSourceStart() == this.getPosition().getSourceStart() && f.getPosition().getSourceEnd() == this.getPosition().getSourceEnd()) {
				return true;
			}
		}
		return false;
	...
	@Override
	@UnsettableProperty
	public <C extends CtRHSReceiver> C setAssignment(CtExpression assignment) {
	...
	@Override
	public <U extends CtLocalVariable> U setInferred(boolean inferred) {
	...
	@Override
	public CtLocalVariable clone() {
		return ((CtLocalVariable) (super.clone()));
	}
