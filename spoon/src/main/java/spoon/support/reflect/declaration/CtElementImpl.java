...
public abstract class CtElementImpl implements CtElement {
	...
	List<CtAnnotation> annotations = emptyList();
	...
	public <A extends Annotation> A getAnnotation(Class<A> annotationType) {
		...
		for (CtAnnotation a : getAnnotations()) {
			if (a.getAnnotationType().getQualifiedName().equals(annot.getQualifiedName())) {
				return ((A) (((CtAnnotation) (a)).getActualAnnotation()));// warning, here we do heavy and costly work with proxy
	...
	@Override
	@SuppressWarnings("unchecked")
	public <A extends Annotation> CtAnnotation getAnnotation(CtTypeReference annotationType) {
		for (CtAnnotation a : getAnnotations()) {
			if (a.getAnnotationType().equals(annotationType)) {
				return ((CtAnnotation) (a));
			}
		}
		return null;
	}
	...
	@Override
	public <E extends CtElement> E setAnnotations(List<CtAnnotation> annotations) {
	...
	@Override
	public <E extends CtElement> E addAnnotation(CtAnnotation annotation) {
		if (annotation == null) {
			return ((E) (this));
		}
		if (this.annotations == CtElementImpl.<CtAnnotation>emptyList()) {
	...
	@Override
	public boolean removeAnnotation(CtAnnotation annotation) {
		if (this.annotations == CtElementImpl.<CtAnnotation>emptyList()) {
			return false;
		}
	...
	public <E extends CtElement> List<E> getAnnotatedChildren(Class<? extends Annotation> annotationType) {
		return ((List<E>) (Query.getElements(this, new AnnotationFilter<>(spoon.reflect.declaration.CtElement.class, annotationType))));
	}
