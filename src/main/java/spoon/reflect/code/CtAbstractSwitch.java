...
public interface CtAbstractSwitch extends CtElement {
	...
	<T extends CtAbstractSwitch> T setSelector(CtExpression selector);

	/**
	 * Gets the list of cases defined for this switch.
	 */
	@PropertyGetter(role = CASE)
	List<CtCase> getCases();

	/**
	 * Sets the list of cases defined for this switch.
	 */
	@PropertySetter(role = CASE)
	<T extends CtAbstractSwitch> T setCases(List<CtCase> cases);

	/**
	 * Adds a case;
	 */
	@PropertySetter(role = CASE)
	<T extends CtAbstractSwitch> T addCase(CtCase c);
	 ...
	 */
	@PropertySetter(role = CASE)
	<T extends CtAbstractSwitch> T addCaseAt(int position, CtCase c);

	/**
	 * Removes a case;
	 */
	@PropertySetter(role = CASE)
	boolean removeCase(CtCase c);
}
