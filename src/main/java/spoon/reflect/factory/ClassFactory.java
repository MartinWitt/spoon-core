...
public class ClassFactory extends TypeFactory {
	 ...
	 */
	public <T> CtClass create(CtClass declaringClass, String simpleName) {
		CtClass c = factory.Core().createClass();
	 ...
	 *            the simple name
	 */
	public <T> CtClass create(CtPackage owner, String simpleName) {
		CtClass c = factory.Core().createClass();
		c.setSimpleName(simpleName);
		owner.addType(c);
		return c;
	 ...
	 */
	public <T> CtClass create(String qualifiedName) {
	...
	@Override
	@SuppressWarnings("unchecked")
	public <T> CtClass get(String qualifiedName) {
		try {
