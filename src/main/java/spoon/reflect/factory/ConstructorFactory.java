...
public class ConstructorFactory extends ExecutableFactory {
	...
	public <T> CtConstructor create(CtClass target, CtConstructor source) {
		CtConstructor newConstructor = ((CtConstructor) (source.clone()));
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> CtConstructor create(CtClass target, CtMethod source) {
		CtMethod method = ((CtMethod) (source.clone()));
		CtConstructor newConstructor = factory.Core().createConstructor();
	 ...
	 * 		the thrown types
	 */
	public <T> CtConstructor create(CtClass target, Set<ModifierKind> modifiers, List<CtParameter> parameters,
	...
	public <T> CtConstructor create(CtClass target, Set<ModifierKind> modifiers, List<CtParameter> parameters,
		...
		CtConstructor constructor = create(target, modifiers, parameters, thrownTypes);
	 ...
	 */
	public <T> CtExecutableReference createReference(CtConstructor c) {
	 ...
	 */
	public <T> CtExecutableReference createReference(Constructor<T> constructor) {
		CtTypeReference type = factory.Type().createReference(constructor.getDeclaringClass());
	 ...
	 */
	public <T> CtExecutableReference createReference(CtTypeReference type, CtExpression... parameters) {
