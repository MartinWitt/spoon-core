...
public class ExecutableFactory extends SubFactory {
	 ...
	 */
	public <T> CtExecutableReference createReference(CtExecutable e) {
		CtExecutableReference er = createReferenceInternal(e);
		er.setParent(e);
		return er;
	}

	private <T> CtExecutableReference createReferenceInternal(CtExecutable e) {
		...
		if (e instanceof CtMethod) {
			...
			return createReference(((CtMethod) (e)).getDeclaringType().getReference(), isStatic, ((CtMethod) (e)).getType().clone(), executableName, refs);
		} else if (e instanceof CtLambda) {
			CtMethod lambdaMethod = ((CtLambda) (e)).getOverriddenMethod();
		...
		// constructor
		return createReference(((CtConstructor) (e)).getDeclaringType().getReference(), ((CtConstructor) (e)).getType().clone(), CtExecutableReference.CONSTRUCTOR_NAME, refs);
	}
	 ...
	 */
	public <T> CtExecutableReference createReference(CtTypeReference declaringType, CtTypeReference type, String methodName, CtTypeReference... parameterTypes) {
	 ...
	 * 		list of parameter's types
	 */
	public <T> CtExecutableReference createReference(CtTypeReference declaringType, boolean isStatic, CtTypeReference type, String methodName, CtTypeReference... parameterTypes) {
	 ...
	 * 		list of parameter's types
	 */
	public <T> CtExecutableReference createReference(CtTypeReference declaringType, boolean isStatic, CtTypeReference type, String methodName, List<CtTypeReference> parameterTypes) {
		CtExecutableReference methodRef = factory.Core().createExecutableReference();
	 ...
	 * 		list of parameter's types
	 */
	public <T> CtExecutableReference createReference(CtTypeReference declaringType, CtTypeReference type, String methodName, List<CtTypeReference> parameterTypes) {
		CtExecutableReference methodRef = factory.Core().createExecutableReference();
		methodRef.setDeclaringType(declaringType);
		methodRef.setSimpleName(methodName);
		methodRef.setType(type);
	 ...
	 */
	public <T> CtExecutableReference createReference(String signature) {
		CtExecutableReference executableRef = factory.Core().createExecutableReference();
