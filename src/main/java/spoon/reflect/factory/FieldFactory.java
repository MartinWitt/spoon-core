...
public class FieldFactory extends SubFactory {
	 ...
	 * 		the field's name
	 */
	public <T> CtField create(CtType target, Set<ModifierKind> modifiers, CtTypeReference type, String name) {
	 ...
	 */
	public <T> CtField create(CtType target, Set<ModifierKind> modifiers, CtTypeReference type, String name, CtExpression defaultExpression) {
	 ...
	 */
	public <T> CtField create(CtType target, CtField source) {
		CtField newField = source.clone();
		if (target != null) {
	 ...
	 */
	public <T> CtFieldReference createReference(CtField field) {
		final CtFieldReference reference = createReference(factory.Type().createReference(field.getDeclaringType()), field.getType().clone(), field.getSimpleName());
	 ...
	 */
	public <T> CtFieldReference createReference(CtTypeReference declaringType, CtTypeReference type, String fieldName) {
	...
	public <T> CtFieldReference createReference(Field field) {
		CtFieldReference fieldRef = factory.Core().createFieldReference();
		fieldRef.setSimpleName(field.getName());
		fieldRef.setDeclaringType(factory.Type().createReference(field.getDeclaringClass()));
		CtTypeReference t = factory.Type().createReference((Class<T>) field.getType());
		fieldRef.setType(t);
		return fieldRef;
	}
	 ...
	 */
	public <T> CtFieldReference createReference(String signature) {
		CtFieldReference fieldRef = factory.Core().createFieldReference();
