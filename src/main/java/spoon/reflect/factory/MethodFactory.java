...
public class MethodFactory extends ExecutableFactory {
	 ...
	 * 		the method's body
	 */
	public <R, B extends R> CtMethod create(CtClass target, Set<ModifierKind> modifiers, CtTypeReference returnType, String name, List<CtParameter> parameters,
			Set<CtTypeReference> thrownTypes, CtBlock body) {
		CtMethod method = create(target, modifiers, returnType, name, parameters, thrownTypes);
		method.setBody(body);
		return method;
	}
	 ...
	 */
	public <T> CtMethod create(CtType target, CtMethod source, boolean redirectReferences) {
		CtMethod newMethod = source.clone();
	 ...
	 * 		the thrown types
	 */
	public <T> CtMethod create(CtType target, Set<ModifierKind> modifiers, CtTypeReference returnType, String name, List<CtParameter> parameters,
			Set<CtTypeReference> thrownTypes) {
	 ...
	 */
	public <T> CtExecutableReference createReference(CtMethod m) {
	...
	public <T> CtExecutableReference createReference(Method method) {
		return createReference(factory.Type().createReference(method.getDeclaringClass()), ((CtTypeReference) (factory.Type().createReference(method.getReturnType()))), method.getName(),
	...
	public Collection<CtMethod> getMainMethods() {
		...
		for (CtType t : factory.Type().getAll()) {
			if (t instanceof CtClass) {
				CtMethod m = ((CtClass) (t)).getMethod(factory.Type().createReference(void.class), "main", factory.Type().createArrayReference(factory.Type().createReference(String.class)));
