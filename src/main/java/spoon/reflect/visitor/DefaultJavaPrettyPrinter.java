...
public class DefaultJavaPrettyPrinter implements CtVisitor, PrettyPrinter {
	...
	@Override
	public <A extends Annotation> void visitCtAnnotation(CtAnnotation annotation) {
	...
	@Override
	public <A extends Annotation> void visitCtAnnotationType(CtAnnotationType annotationType) {
	...
	@Override
	public <T> void visitCtArrayRead(CtArrayRead arrayRead) {
		printCtArrayAccess(arrayRead);
	}

	@Override
	public <T> void visitCtArrayWrite(CtArrayWrite arrayWrite) {
		printCtArrayAccess(arrayWrite);
	}

	private <T, E extends CtExpression> void printCtArrayAccess(CtArrayAccess arrayAccess) {
	...
	@Override
	public <T> void visitCtArrayTypeReference(CtArrayTypeReference reference) {
	...
	@Override
	public <T> void visitCtAssert(CtAssert asserted) {
	...
	@Override
	public <T, A extends T> void visitCtAssignment(CtAssignment assignement) {
	...
	@Override
	public <T> void visitCtBinaryOperator(CtBinaryOperator operator) {
	...
	@Override
	public <R> void visitCtBlock(CtBlock block) {
	...
	public <E> void visitCtCase(CtCase caseStatement) {
		enterCtStatement(caseStatement);
		if (caseStatement.getCaseExpression() != null) {
			printer.writeKeyword("case").writeSpace();
			List<CtExpression> caseExpressions = caseStatement.getCaseExpressions();
			for (int i = 0; i < caseExpressions.size(); i++) {
				CtExpression caseExpression = caseExpressions.get(i);
	...
	public void visitCtCatch(CtCatch catchBlock) {
		...
		CtCatchVariable parameter = catchBlock.getParameter();
	...
	@Override
	public <T> void visitCtClass(CtClass ctClass) {
		...
		if (ctClass.getSimpleName() != null && !CtType.NAME_UNKNOWN.equals(ctClass.getSimpleName()) && !ctClass.isAnonymous()) {
			visitCtType(ctClass);
			printer.writeKeyword("class").writeSpace().writeIdentifier(stripLeadingDigits(ctClass.getSimpleName()));
	...
	@Override
	public <T> void visitCtConditional(CtConditional conditional) {
	...
	@Override
	public <T> void visitCtConstructor(CtConstructor constructor) {
	...
	@Override
	public <T extends Enum<?>> void visitCtEnum(CtEnum ctEnum) {
		visitCtType(ctEnum);
		printer.writeKeyword("enum").writeSpace().writeIdentifier(stripLeadingDigits(ctEnum.getSimpleName()));
	...
	@Override
	public <T> void visitCtExecutableReference(CtExecutableReference reference) {
	...
	@Override
	public <T> void visitCtField(CtField f) {
		...
		if (f.getType() instanceof CtArrayTypeReference) {
			try (Writable unused = context.modify().skipArray(shouldSquareBracketBeSkipped(((CtArrayTypeReference) (f.getType())), CtArrayTypeReferenceImpl.DeclarationKind.TYPE))) {
				scan(f.getType());
			}
		} else {
			scan(f.getType());
		...
		if (f.getType() instanceof CtArrayTypeReference) {
			try (Writable unused = context.modify().skipArray(shouldSquareBracketBeSkipped(((CtArrayTypeReference) (f.getType())), CtArrayTypeReferenceImpl.DeclarationKind.IDENTIFIER))) {
	...
	@Override
	public <T> void visitCtEnumValue(CtEnumValue enumValue) {
	...
	@Override
	public <T> void visitCtFieldRead(CtFieldRead fieldRead) {
		printCtFieldAccess(fieldRead);
	}

	@Override
	public <T> void visitCtFieldWrite(CtFieldWrite fieldWrite) {
		printCtFieldAccess(fieldWrite);
	}

	private <T> void printCtFieldAccess(CtFieldAccess f) {
		enterCtExpression(f);
		try (Writable _context = context.modify()) {
	...
	@Override
	public <T> void visitCtThisAccess(CtThisAccess thisAccess) {
		try {
	...
	@Override
	public <T> void visitCtSuperAccess(CtSuperAccess f) {
		enterCtExpression(f);
		if (f.getTarget() != null) {
			scan(f.getTarget());
			printer.writeSeparator(".");
	...
	public void visitCtImport(CtImport ctImport) {
		if (ctImport.getImportKind() != null) {
			...
			ctImport.accept(new CtImportVisitor() {
				@Override
				public <T> void visitTypeImport(CtTypeReference typeReference) {
					writeImportReference(typeReference);
				}

				@Override
				public <T> void visitMethodImport(CtExecutableReference execRef) {
				...
				@Override
				public <T> void visitFieldImport(CtFieldReference fieldReference) {
					printer.writeKeyword("static");
					printer.writeSpace();
	...
	@Override
	public <T> void visitCtAnnotationFieldAccess(CtAnnotationFieldAccess annotationFieldAccess) {
	...
	@Override
	public <T> void visitCtFieldReference(CtFieldReference reference) {
	...
	@Override
	public <T> void visitCtInterface(CtInterface intrface) {
		visitCtType(intrface);
		printer.writeKeyword("interface").writeSpace().writeIdentifier(stripLeadingDigits(intrface.getSimpleName()));
	...
	@Override
	public <T> void visitCtInvocation(CtInvocation invocation) {
		...
		if (invocation.getExecutable().isConstructor()) {
			...
			CtType parentType = invocation.getParent(spoon.reflect.declaration.CtType.class);
	...
	public void visitCtTextBlock(CtTextBlock ctTextBlock) {
		...
		printer.writeLiteral(LiteralHelper.getTextBlockToken(ctTextBlock));
		exitCtExpression(ctTextBlock);
	}

	@Override
	public <T> void visitCtLocalVariable(CtLocalVariable localVariable) {
		enterCtStatement(localVariable);
		if (env.isPreserveLineNumbers()) {
			getPrinterHelper().adjustStartPosition(localVariable);
		}
		// for resources in try-with-resources
		if (!context.isNextForVariable() && !localVariable.isImplicit()) {
			...
			if (localVariable.isInferred() && this.env.getComplianceLevel() >= 10) {
				...
				if (localVariable.getType() instanceof CtArrayTypeReference) {
					try (Writable unused = context.modify().skipArray(shouldSquareBracketBeSkipped(((CtArrayTypeReference) (localVariable.getType())), CtArrayTypeReferenceImpl.DeclarationKind.TYPE))) {
		...
		if (localVariable.getType() instanceof CtArrayTypeReference) {
			try (Writable unused = context.modify().skipArray(shouldSquareBracketBeSkipped(((CtArrayTypeReference) (localVariable.getType())), CtArrayTypeReferenceImpl.DeclarationKind.IDENTIFIER))) {
				printSquareBrackets(((CtArrayTypeReference) (localVariable.getType())));
			}
		}
		// for resources in try-with-resources
		if (localVariable.getDefaultExpression() != null && !localVariable.isImplicit()) {
			printer.writeSpace().writeOperator("=").writeSpace();
	...
	@Override
	public <T> void visitCtLocalVariableReference(CtLocalVariableReference reference) {
		printer.writeIdentifier(reference.getSimpleName());
	}

	@Override
	public <T> void visitCtCatchVariable(CtCatchVariable catchVariable) {
		if (env.isPreserveLineNumbers()) {
	...
	@Override
	public <T> void visitCtCatchVariableReference(CtCatchVariableReference reference) {
		printer.writeIdentifier(reference.getSimpleName());
	}

	@Override
	public <T> void visitCtMethod(CtMethod m) {
	...
	@Override
	public <T> void visitCtAnnotationMethod(CtAnnotationMethod annotationMethod) {
	...
	@Override
	@SuppressWarnings("rawtypes")
	public <T> void visitCtNewArray(CtNewArray newArray) {
		enterCtExpression(newArray);
		boolean isNotInAnnotation = newArray.getParent(spoon.reflect.declaration.CtAnnotationType.class) == null && newArray.getParent(spoon.reflect.declaration.CtAnnotation.class) == null;
		if (isNotInAnnotation) {
			...
			for (int i = 0; ref instanceof CtArrayTypeReference; i++) {
				...
				if (newArray.getDimensionExpressions().size() > i) {
					CtExpression e = newArray.getDimensionExpressions().get(i);
					scan(e);
				}
				printer.writeSeparator("]");
	...
	@Override
	public <T> void visitCtConstructorCall(CtConstructorCall ctConstructorCall) {
	...
	@Override
	public <T> void visitCtNewClass(CtNewClass newClass) {
	...
	private <T> void printConstructorCall(CtConstructorCall ctConstructorCall) {
		try (Writable _context = context.modify()) {
	 ...
	 */
	private <T> boolean hasDeclaringTypeWithGenerics(CtTypeReference reference) {
	...
	@Override
	public <T> void visitCtLambda(CtLambda lambda) {
		...
		if (isSingleParameterWithoutExplicitType(lambda) && !ignoreImplicit) {
			elementPrinterHelper.printList(lambda.getParameters(), null, false, "", false, false, ",", false, false, "", this::scan);
		} else {
			elementPrinterHelper.printList(lambda.getParameters(), null, false, "(", false, false, ",", false, false, ")", this::scan);
		}
		printer.writeSpace();
	...
	private <T> boolean isSingleParameterWithoutExplicitType(CtLambda lambda) {
		return lambda.getParameters().size() == 1 && (lambda.getParameters().get(0).getType() == null
				|| lambda.getParameters().get(0).getType().isImplicit());
	}

	@Override
	public <T, E extends CtExpression> void visitCtExecutableReferenceExpression(CtExecutableReferenceExpression expression) {
	...
	@Override
	public <T, A extends T> void visitCtOperatorAssignment(CtOperatorAssignment assignment) {
	...
	@Override
	public <T> void visitCtParameter(CtParameter parameter) {
		...
		if (parameter.isVarArgs()) {
			...
			if (parameter.isInferred() && this.env.getComplianceLevel() >= 11) {
				getPrinterTokenWriter().writeKeyword("var");
			} else {
	...
	private <T> boolean isParameterWithImplicitType(CtParameter parameter) {
		return parameter.getType() != null && !parameter.getType().isImplicit();
	}

	private <T> boolean isNotFirstParameter(CtParameter parameter) {
		return parameter.getParent() != null
				&& parameter.getParent().getParameters().indexOf(parameter) != 0;
	}

	@Override
	public <T> void visitCtParameterReference(CtParameterReference reference) {
		printer.writeIdentifier(reference.getSimpleName());
	}

	@Override
	public <R> void visitCtReturn(CtReturn returnStatement) {
	...
	private <T> void visitCtType(CtType type) {
	...
	private <S> void writeSwitch(CtAbstractSwitch abstractSwitch) {
	...
	@Override
	public <E> void visitCtSwitch(CtSwitch switchStatement) {
	...
	@Override
	public <T, S> void visitCtSwitchExpression(CtSwitchExpression switchExpression) {
	...
	@Override
	public <T> void visitCtIntersectionTypeReference(CtIntersectionTypeReference reference) {
		if (reference.isImplicit()) {
			return;
		}
	...
	@Override
	public <T> void visitCtTypeReference(CtTypeReference ref) {
		visitCtTypeReference(ref, true);
	}

	@Override
	public <T> void visitCtTypeAccess(CtTypeAccess typeAccess) {
	...
	@Override
	public <T> void visitCtUnaryOperator(CtUnaryOperator operator) {
	...
	@Override
	public <T> void visitCtVariableRead(CtVariableRead variableRead) {
	...
	@Override
	public <T> void visitCtVariableWrite(CtVariableWrite variableWrite) {
	...
	@Override
	public <T> void visitCtCodeSnippetExpression(CtCodeSnippetExpression expression) {
	...
	@Override
	public <T> void visitCtUnboundVariableReference(CtUnboundVariableReference reference) {
		printer.writeIdentifier(reference.getSimpleName());
	}
