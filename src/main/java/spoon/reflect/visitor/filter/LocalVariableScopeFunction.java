...
public class LocalVariableScopeFunction implements CtConsumableFunction<CtLocalVariable> {
	...
	public void apply(final CtLocalVariable localVariable, CtConsumer<Object> outputConsumer) {
		localVariable.map(new SiblingsFunction().mode(SiblingsFunction.Mode.NEXT).includingSelf(true)).map(new CtScannerFunction().setListener(this.listener)).select(new Filter<CtElement>() {
			@Override
