...
public class PotentialVariableDeclarationFunction implements CtConsumableFunction<CtElement>, CtQueryAware {
	...
	public void apply(CtElement input, CtConsumer<Object> outputConsumer) {
		...
		input.getFactory().createQuery().map(new SiblingsFunction().mode(SiblingsFunction.Mode.PREVIOUS)).select(new TypeFilter<>(spoon.reflect.declaration.CtVariable.class));
		if (variableName != null) {
			// variable name is defined so we have to search only for variables with that name
			siblingsQuery = siblingsQuery.select(new NamedElementFilter<>(spoon.reflect.declaration.CtNamedElement.class, variableName));
		}
		...
		while (scopeElement != null && !(scopeElement instanceof CtPackage) && scopeElement.isParentInitialized()) {
			...
			if (parent instanceof CtType) {
				...
				CtQuery q = parent.map(new AllTypeMembersFunction(spoon.reflect.declaration.CtField.class));
				...
				if (parent instanceof CtSwitch && scopeElement instanceof CtCase && ((CtCase) (scopeElement)).getCaseKind() == CaseKind.COLON) {
					SiblingsFunction siblingsFunction = new SiblingsFunction().mode(SiblingsFunction.Mode.PREVIOUS);
					List<CtCase> list = input.getFactory().createQuery().map(siblingsFunction).setInput(scopeElement).filterChildren(new TypeFilter<>(spoon.reflect.code.CtCase.class)).list();
					for (CtCase c : list) {
					...
					if (parent instanceof CtBodyHolder || parent instanceof CtStatementList) {
