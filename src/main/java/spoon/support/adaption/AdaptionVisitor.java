...
class AdaptionVisitor extends CtAbstractVisitor {
	...
	public <T> void visitCtArrayTypeReference(CtArrayTypeReference reference) {
		CtTypeReference newArrayType = adapt(reference.getArrayType(), hierarchy);
	...
	public <T> void visitCtIntersectionTypeReference(CtIntersectionTypeReference reference) {
		List<CtTypeReference> newBounds = reference.getBounds().stream().map(it -> adapt(it, hierarchy)).collect(Collectors.toList());
