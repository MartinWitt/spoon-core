...
public class ContextBuilder {
	...
	@SuppressWarnings("unchecked")
	<T> CtLocalVariable getLocalVariableDeclaration(final String name) {
		final Class<CtLocalVariable> clazz = ((Class<CtLocalVariable>) (jdtTreeBuilder.getFactory().Core().createLocalVariable().getClass()));
		final CtLocalVariable localVariable =
				this.<T,  CtLocalVariable>getVariableDeclaration(name, clazz);
	...
	@SuppressWarnings("unchecked")
	<T> CtCatchVariable getCatchVariableDeclaration(final String name) {
		final Class<CtCatchVariable> clazz = ((Class<CtCatchVariable>) (jdtTreeBuilder.getFactory().Core().createCatchVariable().getClass()));
		final CtCatchVariable catchVariable =
				this.<T,  CtCatchVariable>getVariableDeclaration(name, clazz);
	...
	<T> CtVariable getVariableDeclaration(final String name) {
		final CtVariable variable = this.<T,  CtVariable>getVariableDeclaration(name, null);
		if (variable == null) {
			// note: this happens when using the new try(vardelc) structure
	...
	@SuppressWarnings("unchecked")
	private <T, U extends CtVariable> U getVariableDeclaration(
				...
				while (!referenceBindings.isEmpty()) {
					...
					for (final FieldBinding fieldBinding : referenceBinding.fields()) {
						if (name.equals(new String(fieldBinding.readableName()))) {
							...
							U field = ((U) (fieldFactory.create(parentOfField, EnumSet.noneOf(spoon.reflect.declaration.ModifierKind.class), referenceBuilder.getTypeReference(fieldBinding.type), name)));
				...
				if (name.equals(potentialVariable.getSimpleName())) {
					...
					for (final ASTPair astPair : stack) {
						if (astPair.element == parentOfPotentialVariable) {
							...
							if (astPair.element instanceof CtExecutable) {
