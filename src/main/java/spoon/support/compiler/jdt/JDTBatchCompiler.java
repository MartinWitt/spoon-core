...
public class JDTBatchCompiler extends org.eclipse.jdt.internal.compiler.batch.Main {
		...
		for (int round = 0; round < 2; round++) {
			for (CompilationUnit compilationUnit : this.compilationUnits) {
				...
				if (isModuleInfo == (round == 0)) {
					// 1st round: modules, 2nd round others (to ensure populating pathToModName well in time)
					...
					if (isModuleInfo) {
						...
						for (Map.Entry<String, char[]> entry : pathToModName.entrySet()) {
							if (fileName.startsWith(entry.getKey())) {
								// associate CUs to module by common prefix
	...
	public CompilationUnitDeclaration[] getUnits() {
		...
		TreeBuilderCompiler treeBuilderCompiler = new TreeBuilderCompiler(
				...
				this.out, jdtCompiler.getEnvironment().getIgnoreSyntaxErrors(), jdtCompiler.getEnvironment().getLevel(), new CompilationProgress() {
