...
public class JDTCommentBuilder {
	...
	private void insertCommentInAST(final CtComment comment) {
		...
		CtInheritanceScanner insertionVisitor = new CtInheritanceScanner() {
			...
			private <T> void visitInterfaceType(CtType e) {
				final List<CtElement> elements = new ArrayList<>();
				for (CtTypeMember typeMember : e.getTypeMembers()) {
			...
			@Override
			public <T> void scanCtVariable(CtVariable e) {
				e.addComment(comment);
			}

			private <S> void visitSwitch(CtAbstractSwitch e) {
				List<CtCase> cases = e.getCases();
			...
			@Override
			public <A extends Annotation> void visitCtAnnotation(CtAnnotation e) {
