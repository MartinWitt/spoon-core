...
public class ParentExiter extends CtInheritanceScanner {
	...
	@Override
	public <R> void scanCtExecutable(CtExecutable e) {
	...
	@Override
	public <T, E extends CtExpression> void scanCtTargetedExpression(CtTargetedExpression targetedExpression) {
	...
	@Override
	public <T> void scanCtType(CtType type) {
		if (child instanceof CtType && !(child instanceof CtTypeParameter)) {
			...
			if (child instanceof CtEnumValue && type instanceof CtEnum) {
				((CtEnum) (type)).addEnumValue(((CtEnumValue) (child)));
			} else {
				if (child instanceof CtField) {
	...
	@Override
	public <T> void scanCtVariable(CtVariable v) {
	...
	private <T> boolean hasChildEqualsToDefaultValue(CtVariable ctVariable) {
	...
	@Override
	public <A extends java.lang.annotation.Annotation> void visitCtAnnotation(CtAnnotation annotation) {
		if (child instanceof CtExpression) {
	...
	@Override
	public <T> void visitCtConstructor(CtConstructor e) {
	...
	@Override
	public <T> void visitCtMethod(CtMethod e) {
		if (e.getBody() == null && child instanceof CtBlock) {
			e.setBody(((CtBlock) (child)));
			return;
		} else {
	...
	private <T> boolean hasChildEqualsToType(CtMethod ctMethod) {
	...
	private <T> boolean hasChildEqualsToDefaultValue(CtAnnotationMethod ctAnnotationMethod) {
		final AnnotationMethodDeclaration parent = (AnnotationMethodDeclaration) jdtTreeBuilder.getContextBuilder().stack.peek().node;
		// Default value is equals to the jdt child.
	...
	private <T, E extends CtExpression> boolean visitArrayAccess(CtArrayAccess arrayAccess) {
		if (child instanceof CtExpression) {
		...
		if (child instanceof CtExpression) {
			if (operator.getLeftHandOperand() == null) {
				...
				if (operator.getRightHandOperand() == null) {
					...
					if (jdtTreeBuilder.getContextBuilder().stack.peek().node instanceof StringLiteralConcatenation) {
	...
	public <E> void visitCtCase(CtCase caseStatement) {
		...
		if (node instanceof CaseStatement && ((CaseStatement) (node)).constantExpressions != null && child instanceof CtExpression && caseStatement.getCaseExpressions().size() < ((CaseStatement) (node)).constantExpressions.length) {
			caseStatement.addCaseExpression(((CtExpression) (child)));
			return;
		} else {
			if (child instanceof CtStatement) {
	...
	@Override
	public <T> void visitCtCatchVariable(CtCatchVariable e) {
	...
	public <T> void visitCtClass(CtClass ctClass) {
		if (child instanceof CtConstructor) {
			CtConstructor constructor = ((CtConstructor) (child));
	...
	private <T> void fixJdtEnumConstructorSuperCall(CtClass ctClass, CtConstructor constructor) {
	...
	public void visitCtTypeParameter(CtTypeParameter typeParameter) {
		if (childJDT instanceof TypeReference && child instanceof CtTypeAccess) {
			if (typeParameter.getSuperclass() == null) {
				...
				if (typeParameter.getSuperclass() instanceof CtIntersectionTypeReference) {
	...
	public void visitCtFor(CtFor forLoop) {
		if (isContainedInForInit() && child instanceof CtStatement) {
			...
			if (isContainedInForUpdate() && child instanceof CtStatement) {
				...
				if (isContainedInForCondition() && child instanceof CtExpression) {
	...
	@Override
	public <T> void visitCtSuperAccess(CtSuperAccess superAccess) {
		if (child instanceof CtTypeAccess) {
	...
	private <T> boolean setTargetFromUnqualifiedAccess(CtInvocation invocation) {
	...
	private <T> boolean hasChildEqualsToQualification(CtInvocation ctInvocation) {
	...
	private <T> boolean hasChildEqualsToReceiver(CtInvocation ctInvocation) {
		...
		if (childJDT instanceof TypeReference && child instanceof CtTypeAccess) {
			...
			if (child instanceof CtExpression) {
				if (isContainedInDimensionExpression()) {
					...
					if (child instanceof CtNewArray && childJDT instanceof ArrayInitializer && jdtTreeBuilder.getContextBuilder().stack.peek().node instanceof ArrayAllocationExpression) {
	...
	private <T> boolean hasChildEqualsToEnclosingInstance(CtConstructorCall ctConstructorCall) {
	...
	private <T> boolean hasChildEqualsToType(CtConstructorCall ctConstructorCall) {
	...
	public <T, E extends CtExpression> void visitCtExecutableReferenceExpression(CtExecutableReferenceExpression expression) {
		if (child instanceof CtExpression) {
			...
			if (child instanceof CtTypeParameterReference) {
	...
	public <E> void visitCtSwitch(CtSwitch switchStatement) {
		...
		if (child instanceof CtCase) {
			...
			child.setPosition(jdtTreeBuilder.getPositionBuilder().buildPosition(((CtCase) (child))));
			return;
		}
	...
	public <T, S> void visitCtSwitchExpression(CtSwitchExpression switchExpression) {
		if (switchExpression.getSelector() == null && child instanceof CtExpression) {
			switchExpression.setSelector(((CtExpression) (child)));
			return;
		}
		if (child instanceof CtCase) {
			switchExpression.addCase(((CtCase) (child)));
			// we have all statements of the case. Update source position now
			child.setPosition(jdtTreeBuilder.getPositionBuilder().buildPosition(((CtCase) (child))));
			return;
		}
			...
			if (child instanceof CtVariableRead) {
				...
				if (variableRef.getDeclaration() != null) {
					...
					for (ASTPair pair : this.jdtTreeBuilder.getContextBuilder().stack) {
						final List<CtLocalVariable> variables = pair.element.getElements(new TypeFilter<>(spoon.reflect.code.CtLocalVariable.class));
