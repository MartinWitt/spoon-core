...
public class ReferenceBuilder {
	 ...
	 */
	<T> CtTypeReference buildTypeReference(TypeReference type, Scope scope) {
		return buildTypeReference(type, scope, false);
	}
	<T> CtTypeReference buildTypeReference(TypeReference type, Scope scope, boolean isTypeCast) {
		if (type == null) {
			return null;
		}
		CtTypeReference typeReference = this.<T>getTypeReference(type.resolvedType, type);
	 ...
	 * @return
	 */
	<T> CtTypeReference buildTypeReference(QualifiedTypeReference type, Scope scope) {
		CtTypeReference accessedType = buildTypeReference((TypeReference) type, scope);
		final TypeBinding receiverType = type != null ? type.resolvedType : null;
		if (receiverType != null) {
			final CtTypeReference ref = getQualifiedTypeReference(type.tokens, receiverType, receiverType.enclosingType(), new JDTTreeBuilder.OnAccessListener() {
				@Override
	...
	private <T> CtTypeReference buildTypeReferenceInternal(CtTypeReference typeReference, TypeReference type, Scope scope, boolean isTypeCast) {
		if (type == null) {
			return null;
		}
	 ...
	 * @return a type reference.
	 */
	<T> CtTypeReference getQualifiedTypeReference(char[][] tokens, TypeBinding receiverType, ReferenceBinding enclosingType, JDTTreeBuilder.OnAccessListener listener) {
				...
				if (CharOperation.equals(anImport.getImportName()[anImport.getImportName().length - 1], expectedName)) {
					if (anImport.isStatic()) {
						...
						try {
							...
							final CtTypeReference typeReference = getTypeReference(declaringType);
	...
	<T> CtExecutableReference getExecutableReference(ReferenceExpression referenceExpression) {
	...
	<T> CtExecutableReference getExecutableReference(ExplicitConstructorCall explicitConstructor) {
		CtExecutableReference ref = getExecutableReference(explicitConstructor.binding);
		if (ref != null) {
	...
	private <T> CtExecutableReference getExecutableReference(MethodBinding exec) {
		return getExecutableReference(exec, -1, -1);
	}

	<T> CtExecutableReference getExecutableReference(MethodBinding exec, int sourceStart, int sourceEnd) {
		if (exec == null) {
			return null;
		}
	...
	<T> CtExecutableReference getExecutableReference(AllocationExpression allocationExpression) {
	...
	<T> CtExecutableReference getExecutableReference(MessageSend messageSend) {
		...
		CtExecutableReference ref = jdtTreeBuilder.getFactory().Core().createExecutableReference();
	...
	<T> CtTypeReference getTypeReference(TypeBinding binding, TypeReference ref) {
		CtTypeReference ctRef = getTypeReference(binding);
	...
	CtTypeReference getTypeParameterReference(TypeBinding binding, TypeReference ref) {
		CtTypeReference ctRef = getTypeReference(binding);
		if (ctRef != null && isCorrectTypeReference(ref)) {
	...
	private <T> void insertGenericTypesInNoClasspathFromJDTInSpoon(TypeReference original, CtTypeReference type) {
	 ...
	 */
	<T> CtTypeReference getTypeReference(TypeReference ref) {
		if (ref == null) {
			return null;
		}
		CtTypeReference res = null;
	 ...
	 */
	private CtTypeReference getTypeParameterReference(String name) {
		CtTypeReference param = null;
	...
	@SuppressWarnings("unchecked")
	<T> CtTypeReference getTypeReference(TypeBinding binding) {
	 ...
	 */
	<T> CtTypeReference getTypeReference(TypeBinding binding, boolean resolveGeneric) {
		if (binding == null) {
			return null;
		}
	...
	private CtTypeReference getTypeReferenceFromArrayBinding(ArrayBinding binding, boolean resolveGeneric) {
		CtArrayTypeReference ref = this.jdtTreeBuilder.getFactory().Core().createArrayTypeReference();
	...
	<T> CtVariableReference getVariableReference(ProblemBinding binding) {
		CtFieldReference ref = this.jdtTreeBuilder.getFactory().Core().createFieldReference();
		if (binding == null) {
			return ref;
		}
