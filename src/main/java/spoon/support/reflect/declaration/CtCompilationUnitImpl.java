...
public class CtCompilationUnitImpl extends CtElementImpl implements CtCompilationUnit {
		...
		if (output != null) {
			...
			if (base.isDirectory()) {
				for (final CtType type : getDeclaredTypes()) {
					...
					for (final CtType inner : type.getElements(new TypeFilter<>(spoon.reflect.declaration.CtType.class))) {
	...
	public String getOriginalSourceCode() {
		if (originalSourceCode == null && getFile() != null && getFile().exists()) {
			try {
				originalSourceCode = Files.readString(getFile().toPath(), this.getFactory().getEnvironment().getEncoding());
			} catch (Exception e) {
