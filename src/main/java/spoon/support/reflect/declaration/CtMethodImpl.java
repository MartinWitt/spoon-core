 ...
 */
public class CtMethodImpl extends CtExecutableImpl implements CtMethod {
	...
	@Override
	public <C extends CtMethod> C setDefaultMethod(boolean defaultMethod) {
	...
	public Collection<CtMethod> getTopDefinitions() {
		...
		getDeclaringType().map(new AllTypeMembersFunction(spoon.reflect.declaration.CtMethod.class)).forEach((CtMethod m) -> {
		...
		return s.stream().filter(ctMethod -> isTopDefinition(ctMethod, s, context)).collect(Collectors.toList());
	}
