...
public class VisitorPartialEvaluator extends CtScanner implements PartialEvaluator {
	...
	private static <T, R> CtLiteral<R> promoteLiteral(CtTypeReference type, CtLiteral<T> literal) {
		...
		if (left instanceof CtLiteral && right instanceof CtLiteral) {
			...
			switch (operator.getKind()) {
				...
				case LT :
					...
					break;
				case MINUS :
					value = convert(operator.getType(), ((Number) (leftObject)).doubleValue() - ((Number) (rightObject)).doubleValue());
					break;
				case MUL :
					value = convert(operator.getType(), ((Number) (leftObject)).doubleValue() * ((Number) (rightObject)).doubleValue());
					break;
				case DIV :
					try {
						...
						if (isFloatingType(operator.getType())) {
							...
							value = convert(operator.getType(), ((Number) (leftObject)).longValue() / ((Number) (rightObject)).longValue());
						}
	...
	private <T> void visitFieldAccess(CtFieldAccess fieldAccess) {
	...
	public void visitCtIf(CtIf ifElement) {
		CtExpression r = evaluate(ifElement.getCondition());
	...
	public <T> void visitCtInvocation(CtInvocation invocation) {
		...
		if (constant) {
			...
			CtType aType = invocation.getParent(spoon.reflect.declaration.CtType.class);
	...
	public <T> void visitCtLocalVariable(final CtLocalVariable localVariable) {
		CtLocalVariable r = localVariable.clone();
	...
	public <T> void visitCtCatchVariable(CtCatchVariable catchVariable) {
		CtCatchVariable r = catchVariable.clone();
	...
	public <R> void visitCtReturn(CtReturn returnStatement) {
		CtReturn r = returnStatement.getFactory().Core().createReturn();
	...
	private <T> void visitVariableAccess(CtVariableAccess variableAccess) {
	...
	@Override
	public <T, A extends T> void visitCtAssignment(CtAssignment variableAssignment) {
		CtAssignment a = variableAssignment.clone();
	...
	public <T> void visitCtConditional(CtConditional conditional) {
		CtExpression r = evaluate(conditional.getCondition());
		if (r instanceof CtLiteral) {
			CtLiteral<Boolean> l = ((CtLiteral<Boolean>) (r));
			if (l.getValue()) {
			...
			CtConditional ifRes = conditional.getFactory().Core().createConditional();
			ifRes.setCondition(r);
