...
public class CtTypeReferenceImpl extends CtReferenceImpl implements CtTypeReference {
	...
	private Optional<Class<?>> getPrimitiveType(CtTypeReference typeReference) {
		switch (typeReference.getSimpleName()) {
			case "boolean":
				return Optional.of(((Class<?>) (boolean.class)));
			case "byte":
				return Optional.of(((Class<?>) (byte.class)));
			case "double":
				return Optional.of(((Class<?>) (double.class)));
			case "int":
				return Optional.of(((Class<?>) (int.class)));
			case "short":
				return Optional.of(((Class<?>) (short.class)));
			case "char":
				return Optional.of(((Class<?>) (char.class)));
			case "long":
				return Optional.of(((Class<?>) (long.class)));
			case "float":
				return Optional.of(((Class<?>) (float.class)));
			case "void":
				return Optional.of(((Class<?>) (void.class)));
			default:
	...
	@SuppressWarnings("unchecked")
	private Class<?> getClassFromThreadLocalCacheOrLoad(CtTypeReference typeReference) {
		...
		return ((Class<?>) (classByQName.get().computeIfAbsent(qualifiedName, key -> loadClassWithQName(classLoader, qualifiedName))));
	}
	...
	public CtType getTypeDeclaration() {
		CtType t = getFactory().Type().get(getQualifiedName());
		if (t != null) {
			return t;
		}
		try {
	...
	public boolean isSubtypeOf(CtTypeReference type) {
		...
		if (this instanceof CtArrayTypeReference) {
			...
			if (java.lang.reflect.Array.class.getName().equals(type.getQualifiedName())) {
				return true;
			}
		}
	...
	@Override
	public <C extends CtTypeReference> C setDeclaringType(CtTypeReference declaringType) {
	...
	@Override
	public <C extends CtTypeReference> C setPackage(CtPackageReference pack) {
		if (pack != null) {
			pack.setParent(this);
		}
	...
	@Override
	public CtIntersectionTypeReference asCtIntersectionTypeReference() {
	...
	public boolean isLocalType() {
		CtType declaration = this.getDeclaration();
		if (declaration != null) {
	...
	public CtTypeReference getAccessType() {
		...
		CtType contextType = getParent(spoon.reflect.declaration.CtType.class);
		if (contextType == null) {
			return declType;
		}
	...
	@Override
	@DerivedProperty
	public CtTypeReferenceImpl setSimplyQualified(boolean isSimplyQualified) {
		if (pack != null) {
