...
public class JavaReflectionTreeBuilder extends JavaReflectionVisitorImpl {
	...
	public <T, R extends CtType> R scan(Class<T> clazz) {
		CtPackage ctPackage;
	...
	public <T> void visitInterface(Class<T> clazz) {
		final CtInterface ctInterface = factory.Core().createInterface();
	...
	public <T extends Annotation> void visitAnnotationClass(Class<T> clazz) {
		...
		enter(new TypeRuntimeBuilderContext(clazz, ctAnnotationType) {
			...
			public void addMethod(CtMethod ctMethod) {
				final CtAnnotationMethod field = factory.Core().createAnnotationMethod();
	...
	public void visitAnnotation(final Annotation annotation) {
		final CtAnnotation ctAnnotation = factory.Core().createAnnotation();
	...
	public <T> void visitConstructor(Constructor<T> constructor) {
		final CtConstructor ctConstructor = factory.Core().createConstructor();
	...
	public void visitMethod(RtMethod method, Annotation parent) {
		final CtMethod ctMethod = factory.Core().createMethod();
	...
	public void visitField(Field field) {
		final CtField ctField = factory.Core().createField();
		...
		try {
			...
			if (modifiers.contains(ModifierKind.STATIC) && modifiers.contains(ModifierKind.PUBLIC) && (field.getType().isPrimitive() || java.lang.String.class.isAssignableFrom(field.getType()))) {
	...
	public void visitEnumValue(Field field) {
		final CtEnumValue ctEnumValue = factory.Core().createEnumValue();
	...
	public void visitTypeReference(CtRole role, WildcardType type) {
		...
		if (!type.getUpperBounds()[0].equals(java.lang.Object.class)) {
	...
	public <T> void visitTypeReference(CtRole role, Class<T> clazz) {
		final CtTypeReference typeReference = factory.Core().createTypeReference();
	...
	public <T> void visitRecord(Class<T> clazz) {
		...
		enter(new TypeRuntimeBuilderContext(clazz, ctRecord) {
			@Override
			public void addConstructor(CtConstructor ctConstructor) {
				ctRecord.addConstructor(((CtConstructor) (ctConstructor)));
			}
